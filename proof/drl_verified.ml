...
let value_eqb =
  failwith "AXIOM TO BE REALIZED (Sakura.Relation.value_eqb)"
...
let semijoin filter_rel source =
  ...
  (filter matches source.rel_tuples) }
...
let equijoin attrs left right =
  ...
  rel_schema = new_schema; rel_tuples = joined }
...
let project attrs r =
  { rel_name = (append ('p'::('r'::('o'::('j'::('_'::[]))))) r.rel_name);
    ...
    (map (project_tuple attrs) r.rel_tuples) }
...
let rename renames r =
  { rel_name =
    ...
    (map (rename_tuple renames) r.rel_tuples) }
...
let rel_union r1 r2 =
  { rel_name =
    ...
    (app r1.rel_tuples r2.rel_tuples) }
...
let rel_diff r1 r2 =
  ...
  (append ('d'::('i'::('f'::('f'::('_'::[])))))
    (append r1.rel_name (append ('_'::[]) r2.rel_name)));
  rel_schema = r1.rel_schema; rel_tuples =
  (filter (fun t -> negb (in_r2 t)) r1.rel_tuples) }
