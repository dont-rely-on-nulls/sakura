module Make (Storage : Management.Physical.S) = struct
  ...
  let execute
    ...
    | Ast.InsertTuples { relation; tuples } ->
      ...
      let* (db, _, _) = Ops.create_tuples storage db rel tuple_list |> wrap_manip in
      Ok db
    ...
    | Ast.DeleteTuple { relation; attributes } ->
      ...
      let* (db, _) = Ops.retract_tuple storage db rel ~tuple_hash |> wrap_manip in
      Ok db
    ...
    | Ast.Assign { target; body } ->
      ...
      let* (db, _, _) = Ops.create_tuples storage db rel (List.map (retarget target) tuples) |> wrap_manip in
      Ok db
    ...
    | Ast.DeleteWhere { target; predicate } ->
      ...
      let common     = List.filter_map
        (fun (n, _) -> if List.exists (fun (m, _) -> m = n) pred_rel.Relation.schema
                       then Some n else None)
        rel.Relation.schema in
      let* joined    = Alg.equijoin storage common rel pred_rel |> Result.map_error wrap_alg in
      let* to_delete = Alg.project storage (List.map fst rel.Relation.schema) joined
                       |> Result.map_error wrap_alg in
