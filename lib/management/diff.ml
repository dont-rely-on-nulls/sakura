(** Structural delta between two Database.t states.
    Computes what changed from ancestor → target at the relation level. *)
...
let diff ~ancestor ~(target : Database.t) =
  ...
  in
  let target_names =
    Database.get_relation_names target |> StringSet.of_list
  in
  ...
  StringSet.fold (fun name acc ->
    ...
    | false, true ->
      let rel = Option.get (Database.get_relation target name) in
      RelationAdded rel :: acc
    | true, false ->
      RelationRemoved name :: acc
    | true, true ->
      let anc_rel = Option.get (Database.get_relation ancestor name) in
      let tgt_rel = Option.get (Database.get_relation target  name) in
      ...
      if anc_rel.Relation.hash = tgt_rel.Relation.hash then acc
        ...
        let added_tuples   = StringSet.diff tgt_keys anc_keys |> StringSet.elements in
        let removed_tuples = StringSet.diff anc_keys tgt_keys |> StringSet.elements in
        let schema_changed = anc_rel.Relation.schema <> tgt_rel.Relation.schema in
        RelationModified { name; added_tuples; removed_tuples; schema_changed } :: acc
      end
