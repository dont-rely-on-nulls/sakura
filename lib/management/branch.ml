...
module Make (Storage : Physical.S with type error = string) = struct
    ...
    (match Storage.store_raw storage (branch_key name) bytes with
     ...
     | Ok () ->
       ...
       if not (List.mem name names) then
         store_names storage (name :: names);
       Ok ())
  ...
  let list storage =
    let names = load_names storage in
    let branches = List.filter_map (fun name ->
        ...
        (match list storage with
         ...
         | Ok branches ->
           if pos >= List.length branches then Generator.Done
             ...
             in
             Generator.Value
               ( Tuple.make_materialized ~relation:Prelude.Catalog.branch_rel_name ~attributes
               , generator ))
    in
      ...
      | Some 0 ->
        (match get_head storage with
         ...
         | Ok (Some branch_name) ->
           ...
           in
           Generator.Value
             ( Tuple.make_materialized ~relation:Prelude.Catalog.head_rel_name ~attributes
