(** Predefined fundamental domains, ported from the Erlang implementation.
    Each domain carries a generator (for enumeration) and membership criteria
    (for validation). Values are stored as {!Conventions.AbstractValue.t} via
    [Obj.magic], following the same convention as {!Standard}. *)

(** Bijection from natural number to integer: 0→0, 1→1, 2→-1, 3→2, 4→-2, … *)
let nat_to_int n =
  if n = 0 then 0
  else if n mod 2 = 1 then (n + 1) / 2
  else -(n / 2)
...
(** Rational number domain: pairs [(numerator, denominator)] where the
    ...
    Cantor pairing on the integers.
...
let rational : Domain.t =
  let rec generator (position : int option) : Generator.result =
    ...
    | Some n ->
      ...
      let den =
        if b mod 2 = 0 then b / 2 + 1 else -((b + 1) / 2)
      in
      let attributes =
