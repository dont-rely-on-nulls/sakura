...
module Make(Storage : Management.Physical.S) = struct
  ...
  let const_relation (pairs : (string * Conventions.AbstractValue.t) list) : Relation.t =
    let attrs = List.fold_left (fun acc (k, v) ->
  ...
  let project storage (attrs : string list) rel =
    ...
    let project_tuple = function
      | Tuple.Materialized t ->
        let attrs' = List.fold_left (fun acc k ->
          match Tuple.AttributeMap.find_opt k t.Tuple.attributes with
          | None   -> acc
          | Some v -> Tuple.AttributeMap.add k v acc)
          Tuple.AttributeMap.empty attrs in
        Tuple.Materialized { t with attributes = attrs' }
      | other -> other
    in
    let new_schema = List.filter (fun (n, _) -> List.mem n attrs) rel.Relation.schema in
    let lazy_gen =
      let rec go g pos = match g pos with
        | Generator.Done    -> Generator.Done
        | Generator.Error e -> Generator.Error e
  ...
  let rename storage (renames : (string * string) list) rel =
    ...
    let rename_tuple = function
      | Tuple.Materialized t ->
        let attrs' = Tuple.AttributeMap.fold (fun k v acc ->
          Tuple.AttributeMap.add (rename_key k) v acc)
          t.Tuple.attributes Tuple.AttributeMap.empty in
        Tuple.Materialized { t with attributes = attrs' }
      | other -> other
    in
    let gen = to_generator storage rel in
    let new_schema = List.map (fun (n, d) -> (rename_key n, d)) rel.Relation.schema in
    let lazy_gen =
      let rec go g pos = match g pos with
        | Generator.Done    -> Generator.Done
        | Generator.Error e -> Generator.Error e
  ...
  let equijoin storage (attrs : string list) left right =
    ...
    | Ok right_tuples ->
      ...
      let get_vals attr_list = function
        | Tuple.Materialized t ->
          List.map (fun a -> Tuple.AttributeMap.find_opt a t.Tuple.attributes) attr_list
        | _ -> List.map (fun _ -> None) attr_list
      in
      let merge lt rt = match lt, rt with
        | Tuple.Materialized l, Tuple.Materialized r ->
          Tuple.Materialized { l with attributes =
            Tuple.AttributeMap.union (fun _ a _ -> Some a)
      ...
      let join_gen =
        let rec chain_with_cont g cont pos =
          ...
          | Generator.Value (t, next) -> Generator.Value (t, fun p -> chain_with_cont next cont p)
          | Generator.Error e         -> Generator.Error e
        in
        let rec from_left lgen lpos =
          ...
          | Generator.Value (lt, next_left) ->
            ...
            let matches = List.filter_map (fun rt ->
              if get_vals attrs rt = lv then Some (merge lt rt) else None) right_tuples in
            (match matches with
  ...
  let attrs_equal
      (m1 : Attribute.materialized Tuple.AttributeMap.t)
      (m2 : Attribute.materialized Tuple.AttributeMap.t) =
    Tuple.AttributeMap.equal
      (fun a b -> Stdlib.(=) a.Attribute.value b.Attribute.value) m1 m2
  ...
  let diff storage rel1 rel2 =
    ...
    | Error e -> Error e
    | Ok right_tuples ->
      let right_mats = List.filter_map (function
        | Tuple.Materialized t -> Some t | _ -> None) right_tuples in
      let left_gen = to_generator storage rel1 in
      let not_in_right = function
        | Tuple.Materialized t ->
          not (List.exists (fun r ->
            attrs_equal t.Tuple.attributes r.Tuple.attributes) right_mats)
        | _ -> true
      in
      let lazy_gen =
        let rec go g pos =
  ...
  let materialize storage rel : (Tuple.materialized list, error) Result.t =
    ...
    | Error e -> Error e
    | Ok tuples ->
      Ok (List.filter_map (function Tuple.Materialized t -> Some t | _ -> None) tuples)
