(** Hash computation for tuples and relations.
    ...
    All hashes are deterministic - the same content always produces the same hash.
    This enables:
...
(** Compute hash for a tuple.
    The hash is computed from:
    - The relation name
    - All attribute names and their marshalled values (sorted by name for determinism) *)
let hash_tuple (tuple : Tuple.materialized) : Conventions.Hash.t =
  ...
  in
  let content =
    List.fold_left (fun acc (name, attr) ->
...
let hash_relation ~name ~(schema : Schema.t) ~tree : Conventions.Hash.t =
  ...
  let tree_hash = match Merkle.root_hash tree with
    | Some h -> h
    | None -> "empty"
  in
