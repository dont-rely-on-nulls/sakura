(** Envelope parser for tagged S-expressions.
    ...
    Parses commands of the form (tag payload...) where the tag selects
    which sublanguage handles the payload. The tag set is open — future
    sublanguages (tcl, ppl, acl) just add new tags. *)
