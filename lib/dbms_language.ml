(** Composable dispatch over sublanguages.
    ...
    All commands must use the tagged envelope format: (tag payload) where [tag]
    is the sublanguage name (drl, dml, ddl, icl, dcl, ...). Adding a new
    sublanguage is just extending the list passed to [create]. *)
...
module Make (Storage : Management.Physical.S with type error = string) = struct
  ...
  let execute (langs : dispatch) (storage : Storage.t)
      (db : Management.Database.t) (input : string) :
      (Sublanguage.result, dispatch_error) Result.t =
    let no_match s =
      Error (NoMatch Sexplib.Sexp.(List [ Atom "parse-error"; Atom s ]))
    in
