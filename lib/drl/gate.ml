(** Finiteness gate for DRL queries.
    ...
    Converts Sakura types to Coq-extracted types and calls the verified
    [predicted_finite] function. Rejects queries whose result cannot be
    statically proven finite. *)
...
let rec query_of_ast : Ast.query -> Drl_verified.query = function
  | Ast.Base name -> Drl_verified.Base (string_to_chars name)
  | Ast.Const pairs ->
      Drl_verified.Const
        (List.map
           (fun (k, v) -> (string_to_chars k, value_of_ast_value v))
           pairs)
  ...
  | Ast.Rename (renames, q) ->
      Drl_verified.Rename
        ( List.map (fun (o, n) -> (string_to_chars o, string_to_chars n)) renames,
          query_of_ast q )
  | Ast.Union (q1, q2) -> Drl_verified.Union (query_of_ast q1, query_of_ast q2)
  | Ast.Diff (q1, q2) -> Drl_verified.Diff (query_of_ast q1, query_of_ast q2)
  | Ast.Take (n, q) -> Drl_verified.Take (n, query_of_ast q)
...
let is_finite (c : Conventions.Cardinality.t) : bool =
  ...
  | Conventions.Cardinality.Finite _ | Conventions.Cardinality.ConstrainedFinite
    ->
      true
  | Conventions.Cardinality.AlephZero | Conventions.Cardinality.Continuum ->
      false
...
let admit (db : Management.Database.t) (ast : Ast.query) : (unit, string) result
  ...
  | None -> Error "referenced relation not found in finiteness check"
