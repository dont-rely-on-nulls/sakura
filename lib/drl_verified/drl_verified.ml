...
let filter_schema attrs s =
  filter (fun p -> existsb (eqb (fst p)) attrs) s
...
(** val rename_in_schema :
    (attr_name * attr_name) list -> schema -> schema **)
...
| [] -> None
| p :: rest ->
  let (a', v) = p in if eqb a a' then Some v else tuple_lookup a rest

(** val project_tuple : attr_name list -> tuple -> tuple **)

let project_tuple attrs t =
  filter (fun p -> existsb (eqb (fst p)) attrs) t
  ...
  forallb (fun a ->
    ...
    | Some v1 ->
      (match tuple_lookup a t2 with
       ...
       | None -> false)
    | None -> (match tuple_lookup a t2 with
               | Some _ -> false
               | None -> true))
    attrs
...
| [] -> None
| p :: rest ->
  let (n, r) = p in if eqb name n then Some r else db_lookup name rest
...
| Base name ->
  (match db_lookup name db with
   | Some r -> Some r.rel_schema
   | None -> None)
| Const pairs ->
  Some
    (map (fun p -> ((fst p),
      ('a'::('b'::('s'::('t'::('r'::('a'::('c'::('t'::[])))))))))) pairs)
...
let literal_to_value = (function LitInt n -> Obj.repr n | LitStr s -> Obj.repr s | LitBool b -> Obj.repr b)
...
let eval_const pairs =
  ...
  let sch =
    map (fun p -> ((fst p),
      ('a'::('b'::('s'::('t'::('r'::('a'::('c'::('t'::[])))))))))) pairs
  in
  { rel_name = ('c'::('o'::('n'::('s'::('t'::[]))))); rel_schema = sch;
  rel_tuples = (tuple0 :: []); rel_finite = true }

(** val semijoin : relation -> relation -> relation **)

let semijoin filter_rel source =
  let common =
    filter (fun a -> has_attr a filter_rel.rel_schema)
      (attr_names source.rel_schema)
  in
  let matches = fun t ->
    existsb (fun ft -> tuples_agree_on common t ft) filter_rel.rel_tuples
  in
  { rel_name = source.rel_name; rel_schema = source.rel_schema; rel_tuples =
  (filter matches source.rel_tuples); rel_finite = source.rel_finite }
...
let equijoin attrs left right =
  ...
  let new_schema =
    merge_schema left.rel_schema
      (remove_attrs (attr_names left.rel_schema)
        (remove_attrs attrs right.rel_schema))
  in
  { rel_name =
  (append ('j'::('o'::('i'::('n'::('_'::[])))))
    (append left.rel_name (append ('_'::[]) right.rel_name)));
  rel_schema = new_schema; rel_tuples = joined; rel_finite =
  ((&&) left.rel_finite right.rel_finite) }

(** val cartesian : relation -> relation -> relation **)

let cartesian left right =
  let joined =
    flat_map (fun lt -> map (fun rt -> merge_tuples lt rt) right.rel_tuples)
      left.rel_tuples
  in
  { rel_name =
  (append ('c'::('a'::('r'::('t'::('_'::[])))))
    (append left.rel_name (append ('_'::[]) right.rel_name)));
  rel_schema = (merge_schema left.rel_schema right.rel_schema); rel_tuples =
  joined; rel_finite = ((&&) left.rel_finite right.rel_finite) }

(** val project : attr_name list -> relation -> relation **)

let project attrs r =
  { rel_name = (append ('p'::('r'::('o'::('j'::('_'::[]))))) r.rel_name);
    rel_schema = (filter_schema attrs r.rel_schema); rel_tuples =
    (map (project_tuple attrs) r.rel_tuples); rel_finite = r.rel_finite }

(** val rename : (attr_name * attr_name) list -> relation -> relation **)

let rename renames r =
  { rel_name =
    (append ('r'::('e'::('n'::('a'::('m'::('e'::('_'::[]))))))) r.rel_name);
    rel_schema = (rename_in_schema renames r.rel_schema); rel_tuples =
    (map (rename_tuple renames) r.rel_tuples); rel_finite = r.rel_finite }
...
let rel_union r1 r2 =
  { rel_name =
    ...
    rel_schema = r1.rel_schema; rel_tuples =
    (app r1.rel_tuples r2.rel_tuples); rel_finite =
    ((&&) r1.rel_finite r2.rel_finite) }

(** val rel_diff : relation -> relation -> relation **)

let rel_diff r1 r2 =
  let in_r2 = fun t ->
    existsb (fun t2 -> tuples_agree_on (attr_names r1.rel_schema) t t2)
      r2.rel_tuples
  in
  { rel_name =
  (append ('d'::('i'::('f'::('f'::('_'::[])))))
    (append r1.rel_name (append ('_'::[]) r2.rel_name)));
  rel_schema = r1.rel_schema; rel_tuples =
  (filter (fun t -> negb (in_r2 t)) r1.rel_tuples); rel_finite =
  r1.rel_finite }
...
let take n r =
  { rel_name = (append ('t'::('a'::('k'::('e'::('_'::[]))))) r.rel_name);
    rel_schema = r.rel_schema; rel_tuples = (firstn n r.rel_tuples);
    rel_finite = true }
