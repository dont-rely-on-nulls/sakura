...
*** DONE Attribute-Level Provenance Tracking
    ...
    - [X] Opt-in via '_provenance' flag in iterators
    - [X] Automatic provenance merging in joins
    - [X] Conflict resolution for duplicate attributes
    - [X] Backward compatible (no overhead when disabled)
    - [X] Foundation for tuple-level lineage tracking (planned)
...
*** TODO Tuple-Level Lineage Tracking
...
**** Provenance vs. Lineage
    ...
    - *Lineage* (PLANNED): Tracks complete operation tree that created the tuple
      #+BEGIN_SRC erlang
      meta => #{
...
**** Implementation Plan
    ...
    1. [ ] Extend =tuple_metadata()= type with lineage field
    2. [ ] Add lineage construction to each iterator operator
       - [ ] select_iterator: ={select, Predicate, ChildLineage}=
       - [ ] project_iterator: ={project, Attributes, ChildLineage}=
       - [ ] equijoin_iterator: ={join, Attr, LeftLineage, RightLineage}=
       - [ ] theta_join_iterator: ={theta_join, Pred, LeftLineage, RightLineage}=
       - [ ] sort_iterator: ={sort, CompareFun, ChildLineage}=
       - [ ] take_iterator: ={take, N, ChildLineage}=
    3. [ ] Implement lineage merging in joins (combine operation trees)
    4. [ ] Add lineage querying API for provenance analysis
    5. [ ] Use lineage for updatable view update propagation
    6. [ ] Add comprehensive lineage tests
